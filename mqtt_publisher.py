...
def _state_payload(reading, temp_offset):
    ...
    ``temp_offset`` compensation as the OLED page and is then snapped to the nearest step via the
    shared :func:`config.snap_temperature` (so the panel and HA agree exactly); humidity and
    pressure are published as whole units (0.1 hPa is below meaningful barometer accuracy — just
    noise here). A sensor that failed this cycle (its field is ``None``) is omitted, so HA keeps
    its last value instead of flipping to *unknown* on a momentary glitch. Returns ``{}`` when
    nothing is readable.
    """
    state = {}
    if reading.temp_c is not None:
        state["temperature"] = config.snap_temperature(reading.temp_c + temp_offset)
